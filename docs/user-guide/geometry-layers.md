...
ras2cng can extract up to 20 HDF geometry layers from HEC-RAS project files. This page describes each layer, its source file type, and its geometry.
...
| `river_reaches` | `HdfXsec.get_river_reaches()` | LineString | River/reach network geometry as represented by the geometry HDF. |
| `edge_lines` | `HdfXsec.get_river_edge_lines()` | LineString | Left and right 1D river-edge lines used by the model footprint API. |
...
| `storage_areas` | `HdfStruc.get_storage_area_polygons()` | Polygon | Storage-area perimeter polygons. |
...
| `pump_stations` | `HdfPump.get_pump_stations()` | Point | Pump-station locations and identifiers. |
| `mannings_n_regions` | `HdfLandCover.get_mannings_region_polygons()` | Polygon | Geometry-level Manning's n region polygons. |
| `infiltration_regions` | `HdfInfiltration.get_infiltration_region_polygons()` | Polygon | Geometry-level infiltration parameter regions. |
...
| `river_reaches` | ✓ | ✓ | — | River/reach network when present |
| `edge_lines` | ✓ | ✓ | — | 1D river-edge lines when present |
...
| `storage_areas` | ✓ | ✓ | ✓ | HDF and text storage-area polygons |
| `pipe_conduits` | ✓ | ✓ | — | Pipe-network conduits when present |
| `pipe_nodes` | ✓ | ✓ | — | Pipe-network nodes when present |
| `pump_stations` | ✓ | ✓ | — | Pump stations when present |
| `mannings_n_regions` | ✓ | ✓ | — | Manning's n parameter regions when present |
| `infiltration_regions` | ✓ | ✓ | — | Infiltration parameter regions when present |
