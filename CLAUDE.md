...
- `cog.py` — **Shared Cloud Optimized GeoTIFF policy. Every COG this repo writes goes through it, and downstream pipelines (RBFS `clb_tx_webmap`, LWI webmap) import from it so they inherit the same defaults.** `area_matched_cog()` is the main entry. Key behaviours, each of which exists because the obvious implementation is wrong:
  ...
  - **`lerc_max_z_error=` opts into lossy LERC** and is never a default. Measured on a smooth WSE surface: -58.7% at 0.001 ft, **-84.2% at 0.01 ft**, with the bound honoured exactly and the nodata mask untouched (treat as optimistic — real WSE is less smooth than the benchmark). Refused on categorical rasters and on a non-positive tolerance; the tolerance is written to the artifact's tags (`compression_lossy`, `compression_max_z_error`) so the loss travels with the file. Never set it on a raster whose values feed a downstream calculation.
